function [modBsSRRC modStreamSRRC] = ModulatedSRRCPulse(bitStream, alpha, T, K, Am, M, N, B, qBits, PAM_level)
...
modBsSRRC = zeros(1, length(t2));
for i = 1:length(modBsSRRC)
    if t2(i) == 0
        %denominator equals 0
        modBsSRRC(i) = 1 - alpha + 4*alpha/pi;
    elseif abs(t2(i)) == T/(4*alpha)
        %denominator equals 0
        modBsSRRC(i) = (alpha/sqrt(2)) * ((1+(2/pi))*sin(pi/(4*alpha)) + (1-(2/pi))*cos(pi/(4*alpha)));
    else
        modBsSRRC(i) = (sin(pi*t2(i)/T*(1-alpha)) + 4*alpha*t2(i)/T*cos(pi*t2(i)/T*(1+alpha))) / (pi*t2(i)/T*(1-((4*alpha*t2(i)/T)^2)));
    end 
end
...
for i = 1:M*N*B*qBits
    ...
    start = (i-1)*T+1;
    stop  = (i-1)*T+2*K*T+1;
    modStreamSRRC(start:stop) = modStreamSRRC(start:stop) + Am(j,k)*modBsSRRC; %consider removing modBsSP for half-sine pulse  
end
