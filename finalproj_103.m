...
filename = 'bowlofpho.jpg';
qBits = 16;
[Zq,blockrow,blockcol,imrow,imcol,minZ,maxZ] = ImagePreProcess_gray(filename,qBits);
...
figure(9); plot(modStreamHS((1:10*spb)+100*spb)); title('Modulated Half-Sine Pulse')
figure(10); plot(modStreamSRRC((1:10*spb)+100*spb)); title('Modulated SRRC Pulse')

tEye = (2^10+1):2^14; % portion of signal we use to draw eye diagram
% eye diagrams for the output of the modulator
eyediagram(modStreamHS(tEye),spb ,1,16); title('Eye Diagram of Modulated Signal - Half-Sine Pulse')
eyediagram(modStreamSRRC(tEye),spb,1); title('Eye Diagram of Modulated Signal - SRRC Pulse')

%% Channel Effects
noise = 0.5; % noise to be added after channel (power?)
...
eyediagram(SRRC_Channel_out(tEye), spb);title('Eye Diagram of SRRC Signal after Channel')
eyediagram(HS_Channel_out(tEye), spb, 1, 16); title('Eye Diagram of HS Signal after Channel')

% eye diagrams of Modulated signals with noise out of the Channel
eyediagram(Noisy_HS(tEye), spb, 1, 16); title('Eye Diagram of Noisey HS Signal')
eyediagram(Noisy_SRRC(tEye), spb); title('Eye Diagram of Noisey SRRC Signal')

%% Matched Filter
% Pass Noisy Signals through the Matched Filter
[HS_MF_Out,SRRC_MF_Out] = Matched_Filter(Noisy_HS, Noisy_SRRC, SRRC_Pulse, spb);

eyediagram(HS_MF_Out(tEye), spb); title('Eye Diagram of HS Signal after Matched Filter')
eyediagram(SRRC_MF_Out(tEye), spb); title('Eye Diagram of SRRC Signal after Matched Filter')
...
eyediagram(HS_ZF_Equalizer_Out(tEye), spb); title('Eye Diagram Output of ZF Equalizer HS')
eyediagram(SRRC_ZF_Equalizer_Out(tEye), spb); title('Eye Diagram Output of ZF Equalizer SRRC')
...
mmse0 = mmse0(1:80);
figure; stem(mmse0)
title('Impulse Response of the MMSE Equaliser (no noise)')
...
eyediagram(HS_MMSE_Out(tEye), spb); title('HS Output of MMSE Equalizer')
eyediagram(SRRC_MMSE_Out(tEye), spb); title('SRRC Output of MMSE Equalizer')
...
mmse0 = mmse0(1:80);
figure; stem(mmse0)
text = sprintf('Impulse Response of the MMSE Equaliser (noise = %1.2f)', noise);
title(text)
...
%Eye diagrams of SRRC and HS from the MMSE Equalizer
eyediagram(HS_MMSE_Out(tEye), spb); title('HS Output of MMSE Equalizer (with noise)')
eyediagram(SRRC_MMSE_Out(tEye), spb); title('SRRC Output of MMSE Equalizer (with noise)')
